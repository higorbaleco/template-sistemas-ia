...
function getQualityBuckets() {
  ...
  const visible = currentView === "clean" ? quality.clean : quality.all;
  return {
    ...quality,
    visible,
  };
...
function getQualityLabel(result) {
  ...
  if (Array.isArray(result.qualityIssues)) {
    if (result.qualityIssues.includes("very_small_group")) {
      return "Muito pequeno";
    }
    if (result.qualityIssues.includes("small_group")) {
      return "Pequeno";
    }
    if (result.qualityIssues.includes("unknown_members")) {
      return "Sem membros";
    }
    if (result.qualityIssues.includes("low_quality")) {
      return "Suspeito";
    }
  }
...
function updateRenderedResults() {
  ...
  updateCopyPanel(buckets.visible);
...
function renderScan(payload) {
  ...
  const sourceStats = summary.sourceStats || {};
  setLog([
    ...
    `Crawl: ${sourceStats.seeds || 0} seed(s), ${sourceStats.pages || 0} página(s), ${sourceStats.candidates || 0} candidato(s), ${sourceStats.unique || 0} grupo(s) únicos, ${sourceStats.failures || 0} falha(s).`,
    ...(payload.progress || []),
  ]);
  els.btnExport.disabled = !buckets.visible.length;
  els.btnCopy.disabled = !buckets.visible.length;
}
...
async function copyCurrentLinks() {
  const buckets = getQualityBuckets();
  const text = FC.entriesToUrlList(buckets.visible);
  if (!text) {
    appendLog("Nenhum link disponível para copiar na visualização atual.");
    return;
  }

  const ok = await copyTextToClipboard(text);
  if (ok) {
    appendLog("Links da visualização atual copiados para a área de transferência.");
    setViewMode("copy");
  } else {
...
async function runScan() {
  ...
  try {
    ...
    els.btnCopy.disabled = !(getQualityBuckets().visible.length);
  }
}

async function exportCsv() {
  const buckets = getQualityBuckets();
  if (!buckets.visible.length) {
    appendLog("Sem resultados disponíveis na visualização atual para exportar.");
    return;
  }

  const csv = FC.entriesToCsv(buckets.visible);
